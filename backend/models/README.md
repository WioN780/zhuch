...
```json
{"format": "zhuch-mlp", "version": 1, "sizes": [90,64,64,5],
 "sha256": "<hex sha256 of the raw little-endian float32 weight bytes>",
 "weights_b64": "<base64 of those bytes>"}
```

- Architecture: sizes `[90, 64, 64, 5]`, tanh on hidden layers, identity output.
...
Checked in so the deployed server (and Docker image, see `backend/Dockerfile`)
ships with playable bots out of the box. It's produced by the training
pipeline (`training/scripts/`) via an MLflow registry export of the current
best genome, converted to this JSON format and dropped into this directory —
re-promote to update it.
...
The current `champion.json` is a v2 genome (sizes `[90, 64, 64, 5]`, matching
the v2 observation vector with accuracy stats + obstacles — see
`docs/contracts.md`).
